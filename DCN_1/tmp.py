...
class DCN():
        ...
        for l_idx in range(self.ae_num_layers):
            ...
            for e in tqdm(range(num_epochs)):
                ...
                for b_idx in range(num_batches):
                    ...
                    b_loss = torch.sum(b_loss, dim=1, keepdims=False)
                    b_loss = torch.mean(b_loss, dim=0, keepdims=False)
                    b_loss.backward()
                    opt.step()
        ...
        for e in tqdm(range(num_epochs)):
            ...
            for b_idx in range(num_batches):
                ...
                if b_idx % log_interval == 0:
                    print('Loss {:.4f}'.format(loss_val))
            print(' Epoch {} loss {:.4f}'.format(e + 1, np.mean(epoch_loss)))
        print('Entire auto encoder trained')
    ...
    def calc_clus_loss(self, cluster_assignments, z):
        ...
        c_loss = torch.sum(dist, dim=0, keepdim=False)
    ...
    def update_cluster_centroids(self, z, C):
        ...
        counts = counts.cpu()
        _centroids = self.centroids.cpu()
        z0 = z.cpu()
        z1 = z0.repeat(1, self.num_clusters).reshape([-1, self.num_clusters, self.latent_dim])
        ...
        E1 = torch.sum(E, dim=0, keepdim=False).cpu()
        denom = torch.reciprocal(counts.float()).unsqueeze(1)
        E2 = denom * E1

        new_centroids = _centroids - E2
        self.centroids = new_centroids.to(self.device)
        return
    ...
    def train_model(self, data):
        ...
        for e in tqdm(range(1, num_epochs + 1)):
            ...
            for b in range(num_batches):
                ...
                b_cluster_assignments = self.get_cluster_assignments(z.clone().detach())
                ...
                self.update_cluster_centroids(
                    z.clone().detach(), b_cluster_assignments
                )
                torch.cuda.empty_cache()
